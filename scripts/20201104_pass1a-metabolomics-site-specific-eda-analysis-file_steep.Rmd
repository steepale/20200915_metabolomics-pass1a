...
if(norm_order == 'none' & run_sam_norm == T & run_feat_norm == T){
  ...
  stop('Normalization order is feat_sam but only sam is normalizaed')
}
...
SamNorm <- function(norm_df,
  ...
  #     ggtitle(paste0(toupper(tissue),' (Before)')) +
  #     xlab(shared_runs[1]) +
  #     ylab(shared_runs[2]) +
...
cor1 <- stats::cor(shared_sam_mat, method = corr_method) %>% round(digits = 3)
...
# Create an NxN Matrix of correlation
cor1 <- stats::cor(shared_met_mat, method = corr_method) %>% round(digits = 3)
