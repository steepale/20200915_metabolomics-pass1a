...
### Remove Features and Impute Feature Values in Named Datasets
...
### Subset Data for Collaboration (Matrices and Dataframes as R Objects)
...
for(row in 1:nrow(df_shared_casrun)){
  ...
  tissue <- df_shared_casrun[row, "TISSUE"] %>% unlist() %>% as.character()
  shared_runs <- df_shared_casrun[row, "RUNS"] %>% unlist() %>% as.character() %>% 
    str_split(pattern = ';') %>% unlist()
  shared_mets <- df_shared_casrun[row, "METABOLITES_RUN"] %>% unlist() %>% as.character() %>% 
    str_split(pattern = ';') %>% unlist()
  shared_sams <- df_shared_casrun[row, "SAMPLES_RUN"] %>% unlist() %>% as.character() %>% 
    str_split(pattern = ';') %>% unlist()
  named <- "named"
  ...
  comp_id <- df_shared_casrun[row, "COMP_ID"] %>% unlist() %>% as.character()
  ...
  # Account for Mayo Replicates
  if(any(grepl('Mayo', shared_runs)) & tissue %in% c("gastrocnemius","heart","lung",
                                                     "liver","white-adipose")){
    # Subset Data
    ################################################################################
    # First collect the mayo replicates
    ##############
    reps <- countdata_df %>%
      ungroup() %>%
      filter(TISSUE == tissue) %>%
    ...
    #rep <- reps[1]
    for(rep in reps){
      # Kick out one of the mayo reps and save the file
      not_rep <- reps[reps != rep]
      ...
      plot_df <- countdata_df %>%
        ...
        filter(labelid %in% shared_sams) %>%
        filter(STUDY_INSTITUTE_METAB_FAMILY %in% shared_runs) %>%
        mutate(REPLICATE = case_when(grepl(paste0(reps[1],"$"),viallabel) ~ reps[1],
                                     grepl(paste0(reps[2],"$"),viallabel) ~ reps[2],
                                     grepl(paste0(reps[3],"$"),viallabel) ~ reps[3])) %>%
        filter(!(STUDY_INSTITUTE == 'Mayo Clinic' & REPLICATE == not_rep))
      ...
      if(all(colord1 == colord2) & all(roword1 == roword2)){
        # Save the matrices
        mat_file1 <- paste0(WD,'/files_to_share/20201103_named-site-comparison-',comp_id,'-',rep,'_',tissue,'_',nxp_out,'_',shared_runs_out[1],'_steep.RDS')
        mat_file2 <- paste0(WD,'/files_to_share/20201103_named-site-comparison-',comp_id,'-',rep,'_',tissue,'_',nxp_out,'_',shared_runs_out[2],'_steep.RDS')
...
comp_ids <- c('0005')
norm_orders <- c('feat_sam','sam_feat')
norm_logs <- c('log10')
sam_norm_centers <- c('none','median','mean')
sam_norm_scales <- c('none','sd','iqr')
feat_norm_centers <- c('none','median','mean')
feat_norm_scales <- c('none','sd','iqr')
ptm <- proc.time()
i <- 1
...
#   arrange(desc(SXS_RANK), desc(FXF_RANK))
