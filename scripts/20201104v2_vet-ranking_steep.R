...
cd1 <- colnames(rm1) %>% sort()
rd1 <- rownames(rm1) %>% sort()
rm1 <- rm1[rd1,cd1]
cd2 <- colnames(rm2) %>% sort()
rd2 <- rownames(rm2) %>% sort()
rm2 <- rm2[rd2,cd2]
...
# Ensure equal
(round(mm[[1]],6) == round(br3,6)) %>%
  table()
# Esnure equal
(round(mm[[2]],6) == round(um3,6)) %>%
  table()
