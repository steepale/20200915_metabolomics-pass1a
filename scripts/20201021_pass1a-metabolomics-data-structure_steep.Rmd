---
title: "20201021_pass1a-metabolomics-data-structure_steep"
author: "Steep"
date: "10/21/2020"
...
# table(pheno_df$Specimen.Processing.sampletypedescription)
# pheno_df %>%
#   filter(Specimen.Processing.sampletypedescription == 'Liver') %>%
#   select(Key.anirandgroup) %>% table()
...
################################################################################
# Subset Data: Collect Only Shared Samples and Metabolites
################################################################################
...
# Summary Statistics of Density plot distribution
################################################################################
cas_sites <- ac_shared_long_as$cas %>% as.character() %>% unique()
# Iterate through the cas sites and collect vectors
#cas_site <- 'duke'
df_all <- data.frame()
for(cas_site in cas_sites){
  # Collect numeric vector
  num_vec <- ac_shared_long_as %>%
    filter(cas == cas_site) %>%
    select(value) %>% unlist() %>% unname()
  df <- NumericSummaryStats(num_vec)
  row.names(df) <- cas_site
...
# Subset Matrixes
ac_duke_mat_shared_as <- ac_duke_mat_all_as[shared_sams,shared_metabs]
ac_mayo_mat_shared_as <- ac_mayo_mat_all_as[shared_sams,shared_metabs]

# Concatenate the matrixes by column and relabel columns (columns = metabolites)
################################################################################
# all(rownames(ac_duke_mat) == rownames(ac_mayo_mat))
# all(colnames(ac_duke_mat) == colnames(ac_mayo_mat))
ac_duke_mat1 <- ac_duke_mat_shared_as
ac_mayo_mat1 <- ac_mayo_mat_shared_as
rownames(ac_duke_mat1) <- paste0(rownames(ac_duke_mat1),'_DUKE')
rownames(ac_mayo_mat1) <- paste0(rownames(ac_mayo_mat1),'_MAYO')
# Make Samples be columns in product below

ac_concat_mat <- cbind(t(ac_duke_mat1),t(ac_mayo_mat1))
# cor1 <- cor(ac_concat_mat)
# image(cor1)

# Create an NxN Matrix of correlation
if(Corr == 'Pearson'){
        cor1 <- stats::cor(ac_concat_mat, method = 'pearson') %>% round(digits = 3)
}else if(Corr == 'Spearman'){
        cor1 <- stats::cor(ac_concat_mat, method = 'spearman') %>% round(digits = 3)
}
...
melted_cormat2 <- melted_cormat %>%
  filter(!(grepl('MAYO', Var1) & grepl('MAYO', Var2))) %>%
  filter(!(grepl('DUKE', Var1) & grepl('DUKE', Var2))) %>%
  filter(grepl('DUKE', Var1)) %>%
  filter(grepl('MAYO', Var2))
# Widen the dataframwe back to matrix
...
# Plot the heatmap
p_out <- pheatmap(cormat2, color=myColor, breaks=myBreaks, cluster_rows = F, cluster_cols = F)
# The plot needs to be saved for viewing (too large)
heatmap_file <- paste0(WD, '/plots/20201021_sample-by-sample-correlation-heatmap-shared-reciprocal_steep.pdf')
...
dim(ac_concat_mat)
pca <- prcomp(t(ac_concat_mat), scale. = F)
...
# Subset Matrixes
ac_duke_mat1 <- ac_duke_mat_all_as[shared_sams,shared_metabs]
ac_mayo_mat1 <- ac_mayo_mat_all_as[shared_sams,shared_metabs]

# Concatenate the matrixes by column and relabel columns (columns = metabolites)
################################################################################
# all(rownames(ac_duke_mat) == rownames(ac_mayo_mat))
# all(colnames(ac_duke_mat) == colnames(ac_mayo_mat))
colnames(ac_duke_mat1) <- paste0(colnames(ac_duke_mat1),'_DUKE')
colnames(ac_mayo_mat1) <- paste0(colnames(ac_mayo_mat1),'_MAYO')
#rownames(ac_mayo_mat) <- paste0(rownames(ac_mayo_mat),'_MAYO')
ac_concat_mat <- cbind(ac_duke_mat1,ac_mayo_mat1)
# cor1 <- cor(ac_concat_mat)
# image(cor1)
...
p_out <- pheatmap(cormat1, color=myColor, breaks=myBreaks, cluster_rows = T, cluster_cols = T)
# The plot needs to be saved for viewing (too large)
heatmap_file <- paste0(WD, '/plots/20201021_metabolite-by-metabolite-correlation-heatmap-shared_steep.pdf')
save_pheatmap_pdf(p_out, heatmap_file, width = 10, height = 10)
...
p_out <- pheatmap(cormat2, color=myColor, breaks=myBreaks, cluster_rows = F, cluster_cols = F)
# The plot needs to be saved for viewing (too large)
heatmap_file <- paste0(WD, '/plots/20201021_metabolite-by-metabolite-correlation-heatmap-shared-reciprocal_steep.pdf')
save_pheatmap_pdf(p_out, heatmap_file, width = 10, height = 10)

print(paste0("Heatmap saved to: ",heatmap_file))
```

# Hone in on individual metabolites and plot boxplots and scatterplots
# HERE
```{r BoxPlots & ScatterPlots}
################################################################################
####################### Plot the BoxPlots of the Metabolites ###################
################################################################################

# Plot the metabolites in boxplots and compare sets
############################################################
ac_shared_long_as %>%
  ggplot(aes(x = value, y = cas, color = cas)) +
  geom_boxplot() +
...
################################################################################
####################### Plot the Histograms of the Metabolites #################
################################################################################
...
################################################################################
####################### Plot the Scatterplots of the Metabolites ###############
################################################################################

# Plot scatter plots
ac_joined_long_as %>%
  ggplot(aes(x = DUKE, y = MAYO, label = sample)) +
