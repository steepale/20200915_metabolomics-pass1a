...
for(i in 1:78){
  um3[i,] <- (um3[i,]-s.mean.um3[i])/s.sd.um3[i]
  br3[i,] <- (br3[i,]-s.mean.br3[i])/s.sd.br3[i]
  #um3[i,] <- (um3[i,]-s.mean.um3[i])
  #br3[i,] <- (br3[i,]-s.mean.br3[i])
}
...
for(i in 1:52){
  ...
  # um3[,i] <- (um3[,i]-f.mean.um3[i])
  # br3[,i] <- (br3[,i]-f.mean.br3[i])
}
...
# mm[[1]] <- mm[[1]][roword1,colord1]
# mm[[2]] <- mm[[2]][roword2,colord2]
# 
# colnames(mm[[1]]) == colnames(br3)
# rownames(mm[[1]]) == rownames(br3)
# 
# br3[1:4, 1:4]
# mm[[1]][1:4,1:4]
# 
# (round(mm[[1]], 6) == round(br3, 6)) %>% table()
